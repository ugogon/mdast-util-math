...
export function mathFromMarkdown() {
   ...
   * @type {FromMarkdownHandle}
   */
  function exitMathFlowFence(token) {
    // Exit if this is the closing fence.
    if (this.data.mathFlowInside) return
    this.buffer()
    this.data.mathFlowInside = true
    const numDollars = token.end.offset - token.start.offset;
    const node = this.stack[this.stack.length-2];
    assert(node.type === 'Math');
    node.data.dollars = numDollars;
  }
   ...
   * @type {FromMarkdownHandle}
   */
  function enterMathTextSequence(token) {
    // This is unnecessarily called twice: at the beginning and the ending Sequence.
    // Could be checked but makes no difference.
    const numDollars = token.end.offset - token.start.offset;
    const node = this.stack[this.stack.length-2];
